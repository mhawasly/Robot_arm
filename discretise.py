...
def distance_to_obstacle(pos,O):
    #import pdb; pdb.set_trace()
    [x_o,y_o,w_o,h_o]=O
    [x,y]=pos
    return min(smallest_dist_to_segment(x,y,[x_o,x_o+w_o], None, y_o ),\
...
if __name__ == "__main__":
    ...
    for h,path in enumerate(Paths_cs):
        ...
        for t,p in enumerate(path):
            ...
            for o in O:
                ...
                d=distance_to_obstacle([x,y],[x_o,y_o,w_o,h_o])

                z=sum(np.where(d>=spans,1,0))   #qualitative descriptor for o
                q.append(z)
            if len(qpath)==0 or qpath[-1]!=q:
                qpath.append(q)
                if tuple(q) not in Points:
                    Points[tuple(q)]=[]
                if len(qpath)!=1:
                    qpath_times_ends.append(t-1)
                    #import pdb; pdb.set_trace()
                    Points[tuple(qpath[-2])]+=Paths_js[h][qpath_times_starts[-1]:qpath_times_ends[-1]+1]
    ...
    #analyse QPaths for feasible positions of qualitative states
    #for [q,q_times_starts,qpath_times_ends) in QPaths:


    #analyse QPaths for all kinds of transitions
    ...
    while True:
        if All_qpaths2[k][1][0:2]==key:
            #from the same group
            #get the manoeuvres on the boundary of the two qualitative states
            pid=All_qpaths2[k][0]
            end=All_qpaths2[k][1][3]

            P=Paths_js[pid][max(0,end-2):min(end+3,len(Paths_js[pid]))] #local manoeuvres that cross the boundary
            Ps.append(P)
            k+=1
            ...
            if len(Ps)!=0:
                ...
                Manoeuvres[tuple([x for y in key for x in y])]=Ps
    ...
    pickle.dump(Manoeuvres, open('manoeuvres.p','w'))
    pickle.dump(Points, open('points.p','w'))
